...
def loglik_bpd(model, x, floor_plan, room_type, text_condition):
    """Compute the log-likelihood in bits per dim."""
    return - model.log_prob(x, floor_plan=floor_plan, room_type=room_type, text_condition=text_condition).sum() / (math.log(2) * x.shape.numel())
...
def elbo_bpd(model, x, floor_plan, room_type, text_condition):
    """
    Compute the ELBO in bits per dim.
    Same as .loglik_bpd(), but may improve readability.
    """
    return loglik_bpd(model, x, floor_plan, room_type, text_condition)
